import { memoryExperienceUpdateInputSchema } from "@tali/contracts";
...
export default defineHandler(async (event) => {
  ...
  try {
    ...
    return jsonResponse(await (await getMemoryService(event.req)).updateExperience({
      actorId,
      itemId,
      memoryId: memoryIdFromParams(event.context.params),
      update: memoryExperienceUpdateInputSchema.parse(await event.req.json()),
    }));
  } catch (error) {

...
export function MemoryDetailPage({ memoryId }: { memoryId: string }) {
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <header className="flex flex-col gap-5 border-b pb-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><h1 className="truncate font-display text-2xl font-semibold leading-tight tracking-[-0.02em]">{memory.displayName}</h1><MemoryStatus status={memory.status} /></div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              {permissions.canManageMemories ? <DropdownMenuItem variant="edit" onSelect={() => { rename.reset(); setRenameName(memory.displayName); setRenameOpen(true); }}><Pencil />Rename</DropdownMenuItem> : null}
              <DropdownMenuItem onSelect={() => setTab("settings")}><Settings />Settings</DropdownMenuItem>
              {permissions.canPurgeMemories ? <><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onSelect={() => { remove.reset(); setDeleteOpen(true); }}><Trash2 />Delete Memory</DropdownMenuItem></> : null}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </header>
      ...
      <EntitySheet pending={rename.isPending} open={renameOpen} onOpenChange={(open) => { if (!rename.isPending) setRenameOpen(open); }} eyebrow="Durable Memory" title="Rename Memory" description="This changes the product name without changing content or the provider-side Bank." width="md" footer={<><Button variant="outline" disabled={rename.isPending} onClick={() => setRenameOpen(false)}>Cancel</Button><Button variant="edit" disabled={!renameName.trim() || rename.isPending} onClick={() => rename.mutate()}>{rename.isPending ? "Saving…" : "Save name"}</Button></>}>
...
function SettingsTab({ active, agents, embeddingModelReady, embeddingModelsPending, memory, onDelete, onExport, onNotice, onRefresh, permissions }: { active: boolean; agents: Instance[]; embeddingModelReady: boolean; embeddingModelsPending: boolean; memory: MemoryResourceDetailView; onDelete: () => void; onExport: () => void; onNotice: (message: string) => void; onRefresh: () => Promise<unknown>; permissions: ReturnType<typeof useProjectPermissions> }) {
  ...
  const [operation, setOperation] = useState<"bind" | "unbind" | "retry" | { replayId: string } | null>(null);
  ...
  const bind = useMutation({ mutationFn: () => { if (!embeddingModelReady) throw new Error("Configure a validated embedding model before attaching Durable Memory."); const agent = availableAgents.find(({ id }) => id === instanceId); if (!agent || (agent.agentPlatform !== "openclaw" && agent.agentPlatform !== "hermes")) throw new Error("Choose an available OpenClaw or Hermes Agent."); return api.bindMemory(memory.id, { instanceId: agent.id, runtimeType: agent.agentPlatform }); }, onSuccess: async () => { setOperation(null); setInstanceId(""); onNotice("Memory binding is active."); await onRefresh(); } });
  const unbind = useMutation({ mutationFn: () => { if (!memory.activeBinding) throw new Error("No active binding exists."); return api.unbindMemory(memory.id, memory.activeBinding.id); }, onSuccess: async () => { setOperation(null); onNotice("Agent detached. Memory content was retained and is available to reattach."); await onRefresh(); } });
  const retry = useMutation({ mutationFn: () => api.retryMemory(memory.id), onSuccess: async () => { setOperation(null); onNotice("Memory recovery completed."); await onRefresh(); await settings.refetch(); } });
  const replay = useMutation({ mutationFn: (outboxId: string) => api.replayMemoryOutbox(memory.id, outboxId), onSuccess: async () => { setOperation(null); onNotice("Retain event was queued for replay."); await outbox.refetch(); } });
  const provider = settings.data?.provider;
  const actionError = bind.error ?? unbind.error ?? retry.error ?? replay.error;
  const pending = bind.isPending || unbind.isPending || retry.isPending || replay.isPending;
  const openOperation = (next: NonNullable<typeof operation>) => {
    bind.reset(); unbind.reset(); retry.reset(); replay.reset(); setOperation(next);
  };
  const operationAllowed = operation === "bind"
    ? permissions.canManageMemories && !memory.activeBinding && !embeddingModelsPending && embeddingModelReady && availableAgents.some((agent) => agent.id === instanceId)
    : operation === "unbind" ? permissions.canManageMemories && Boolean(memory.activeBinding)
    : operation === "retry" ? permissions.canManageMemories && (memory.status === "provisioning" || memory.status === "degraded")
    : operation ? permissions.canReplayMemoryOutbox && Boolean(outbox.data?.items.some((event) => event.id === operation.replayId && (event.status === "retry" || event.status === "dead_letter"))) : false;
  const operationTitle = operation === "bind" ? "Attach Agent" : operation === "unbind" ? "Detach Agent" : operation === "retry" ? "Retry provisioning" : "Replay retain event";
  const submitOperation = () => {
    if (pending || !operationAllowed) return;
    if (operation === "bind") bind.mutate();
    else if (operation === "unbind") unbind.mutate();
    else if (operation === "retry") retry.mutate();
    else if (operation) replay.mutate(operation.replayId);
  };
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,.72fr)]">
    <div className="space-y-5">
      <Card><CardHeader className="border-b"><CardTitle>Runtime binding</CardTitle><CardDescription>One Memory can have one active primary Agent binding. Detached history is retained.</CardDescription></CardHeader><CardContent className="space-y-4 p-5">{memory.activeBinding ? <div className="flex flex-wrap items-center justify-between gap-4"><div><strong className="text-sm">{agents.find(({ id }) => id === memory.activeBinding?.instanceId)?.name ?? memory.activeBinding.instanceId}</strong><p className="mt-1 text-xs capitalize text-muted-foreground">{memory.activeBinding.runtimeType} · attached {formatMemoryDate(memory.activeBinding.attachedAt)}</p></div>{permissions.canManageMemories ? <ResourceActionsMenu resourceName={memory.displayName} actions={[{ label: "Detach Agent", icon: <Unlink />, onSelect: () => openOperation("unbind") }]} /> : null}</div> : <div className="space-y-4"><MemoryNotice>Unbound. This Memory remains intact and can be attached to another Agent.</MemoryNotice>{permissions.canManageMemories ? <div className="flex flex-col gap-3 sm:flex-row"><Button className="h-11" disabled={embeddingModelsPending || !embeddingModelReady || !availableAgents.length} onClick={() => openOperation("bind")}><Link2 />Attach Agent</Button></div> : null}{permissions.canManageMemories && embeddingModelReady && !availableAgents.length ? <p className="text-xs text-muted-foreground">No unbound OpenClaw or Hermes Agent is currently available.</p> : null}</div>}<div className="border-t pt-4"><h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Binding history</h3>{memory.bindingHistory.length ? <div className="mt-3 divide-y border-y">{memory.bindingHistory.map((binding) => <div key={binding.id} className="grid gap-2 py-3 text-xs sm:grid-cols-[minmax(0,1fr)_7rem_8rem]"><span className="truncate font-mono">{binding.instanceId}</span><span className="capitalize">{binding.runtimeType}</span><span className="capitalize text-muted-foreground">{binding.status}</span></div>)}</div> : <p className="mt-2 text-sm text-muted-foreground">No binding history.</p>}</div></CardContent></Card>
    ...
    <EntitySheet open={operation !== null} pending={pending} onOpenChange={(open) => { if (!open) setOperation(null); }}
      title={operationTitle} description={<>Memory: <strong>{memory.displayName}</strong></>} width="md"
      footer={<><Button variant="outline" disabled={pending} onClick={() => setOperation(null)}>Cancel</Button><Button disabled={pending || !operationAllowed} onClick={submitOperation}>{pending ? "Applying…" : operationTitle}</Button></>}>
      <div className="space-y-4">
        <p className="text-sm leading-6">{operation === "bind" ? "Attach this Memory to one available Agent. Existing retained content remains intact." : operation === "unbind" ? "Detach the active Agent binding. Retained Memory content is not deleted." : operation === "retry" ? "Retry provisioning with the current provider configuration." : "Queue this retain event for replay. This changes delivery state."}</p>
        {operation === "bind" ? <Select value={instanceId} onValueChange={setInstanceId} disabled={pending || embeddingModelsPending || !embeddingModelReady}><SelectTrigger aria-label="Choose Agent for Memory" className="h-11 min-w-0 flex-1"><SelectValue placeholder={embeddingModelsPending ? "Checking embedding model…" : embeddingModelReady ? "Choose an available Agent" : "Embedding model required"} /></SelectTrigger><SelectContent>{availableAgents.map((agent) => <SelectItem key={agent.id} value={agent.id}>{agent.name}<span className="ml-2 capitalize text-muted-foreground">{agent.agentPlatform}</span></SelectItem>)}</SelectContent></Select> : null}
        {operation && typeof operation === "object" ? <p className="break-all font-mono text-xs">{operation.replayId}</p> : null}
...
function InsightRow({ insight }: { insight: MemoryInsight }) { return <article className="border-b p-5 last:border-b-0"><div className="flex items-start gap-3"><BrainCircuit className="mt-0.5 size-4 shrink-0 text-link" /><div><p className="text-sm leading-6">{insight.text}</p><p className="mt-2 text-xs text-muted-foreground">{insight.evidence.length} source{insight.evidence.length === 1 ? "" : "s"} · updated {formatRelativeMemoryDate(insight.updatedAt)}</p></div></div></article>; }

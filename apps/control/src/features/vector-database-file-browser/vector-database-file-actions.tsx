...
export function NewFolderDialog({
  ...
  return (
    <EntitySheet
      ...
      width="md"
      title="New folder"
      description="Create a logical TaskLattice folder in the current Vector Database directory."
      footer={
        <>
          <Button
            variant="outline"
    ...
    >
      <div className="space-y-3">
        <Label htmlFor="new-vector-folder-name" required>
          Folder name
        </Label>
        <Input
          id="new-vector-folder-name"
          className="h-11"
          disabled={pending}
          autoFocus
          required
    ...
    >
      <div className="space-y-4">
        {mode === "rename" ? (
          ...
          <div className="space-y-2">
            <Label htmlFor="vector-destination-folder">
              Destination folder
            </Label>
            <Select
              disabled={pending}
              value={currentParentId ?? "root"}
              onValueChange={(value) =>
                onParentChange(value === "root" ? null : value)
              }
            >
              <SelectTrigger
                id="vector-destination-folder"
...
export function DeleteObjectDialog({
  ...
  return (
    <DeleteEntitySheet
      ...
      description="This permanently deletes data. TaskLattice does not provide a recycle bin for Vector Database files."
      error={error}
      onConfirm={onConfirm}
    >
      <div className="space-y-4">
        ...
        <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-sm border bg-border text-sm">
          ...
          <Impact
            label="Processing"
            value={processingFileCount}
            warning={processingFileCount > 0}
          />
          <Impact
            label="Failed"
            value={failedFileCount}
            warning={failedFileCount > 0}
          />
        </dl>
      </div>
    </DeleteEntitySheet>
...
export function UploadFilesSheet({
  ...
  useEffect(() => {
    if (!open) setInputKey((value) => value + 1);
  }, [open]);
  return (
    <Sheet open={open} onOpenChange={(next) => !pending && onOpenChange(next)}>
      <SheetContent
        ...
        aria-busy={pending}
        className="w-full gap-0 overflow-hidden bg-card sm:max-w-[38rem]"
      >
        <SheetHeader className="border-b px-5 py-5 pr-16">
          ...
          <SheetDescription>
            ...
            Files are queued for parsing and indexing.
          </SheetDescription>
        </SheetHeader>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <label className="flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-primary/40 bg-primary-surface text-center hover:bg-accent focus-within:ring-2 focus-within:ring-ring">
            ...
            <span className="mt-1 text-xs text-muted-foreground">
              PDF, Office, HTML, Markdown, text, or images · 25 MiB each
            </span>
            <input
              key={inputKey}
              className="sr-only"
              ...
              multiple
              required
              accept=".pdf,.docx,.pptx,.xlsx,.html,.htm,.md,.txt,.png,.jpg,.jpeg,.tif,.tiff"
...
export function VectorDocumentActionSheet({
  ...
  const saveMetadata = useMutation({
    mutationFn: () => {
      if (!document || !parsedMetadata.data)
        throw new Error(parsedMetadata.error || "Review the metadata fields.");
      return api.updateVectorDocument(databaseId, document.id, {
        customMetadata: parsedMetadata.data,
      });
    },
    onSuccess: async () => {
  ...
  useEffect(() => {
    if (!open || initialView !== "chunks" || !targetChunkId || !chunks.data)
      return;
    const frame = window.requestAnimationFrame(() => {
      window.document
        .getElementById(chunkElementId(targetChunkId))
  ...
  return (
    <Sheet
      open={open}
      onOpenChange={(next) => !saveMetadata.isPending && onOpenChange(next)}
    >
      <SheetContent
        side="right"
        closeDisabled={saveMetadata.isPending}
        aria-busy={saveMetadata.isPending}
        className="!w-full gap-0 overflow-hidden bg-card sm:!w-[min(96vw,52rem)] sm:!max-w-[52rem] [&>button]:size-11"
      >
        ...
        <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-5 py-6 sm:px-6">
          ...
          {chunks.error ? (
            <ErrorMessage>{chunks.error.message}</ErrorMessage>
          ) : null}
          {detail.data && chunks.data && initialView === "preview" ? (
            <section>
              <h3 className="text-sm font-semibold">Indexed text</h3>
              <p className="mt-1 text-xs text-muted-foreground">
                Combined text from the active Vector Records.
              </p>
              {chunks.data.truncated ? (
                <p className="mt-4 border-l-2 border-amber-500 bg-amber-500/5 px-3 py-2 text-xs text-muted-foreground">
                  This preview contains the first {chunks.data.chunks.length} of{" "}
                  {chunks.data.total} chunks.
                </p>
              ) : null}
              <p className="mt-5 whitespace-pre-wrap text-sm leading-7">
                {fullText || "No indexed text is available."}
              </p>
            </section>
          ) : null}
          {detail.data && chunks.data && initialView === "chunks" ? (
            <section>
              <div className="flex items-end justify-between gap-3 border-b pb-3">
                <div>
                  <h3 className="text-sm font-semibold">Vector Records</h3>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Expand a chunk to inspect its full indexed text.
                  </p>
                </div>
                <span className="text-xs text-muted-foreground">
                  {chunks.data.total} records
                </span>
              </div>
              {chunks.data.truncated ? (
                <p className="border-b bg-amber-500/5 px-3 py-2 text-xs text-muted-foreground">
                  Showing the first {chunks.data.chunks.length} records.
                </p>
              ) : null}
                    ...
                    <Collapsible
                      ...
                      onOpenChange={(next) =>
                        setExpandedChunks((current) => {
                          ...
                          if (next) copy.add(chunk.id);
                          else copy.delete(chunk.id);
                          return copy;
                        })
                      }
                    >
                      <article
                        id={chunkElementId(chunk.id)}
                        className={
                      ...
                      >
                        <CollapsibleTrigger asChild>
                          <button
                            type="button"
                            className="flex min-h-20 w-full items-start gap-3 px-3 py-4 text-left outline-none hover:bg-muted/25 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
                          >
                            <ChevronDown
                            ...
                            <span className="min-w-0 flex-1">
                              ...
                              <span className="mt-2 line-clamp-2 block text-xs leading-5 text-muted-foreground">
                                {chunk.content || "No text"}
                              </span>
                            </span>
                          </button>
                        </CollapsibleTrigger>
                        <CollapsibleContent>
                          <div className="border-t bg-muted/15 px-10 py-4">
          ...
          {detail.data && initialView === "metadata" ? (
            <section>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Typed file-level fields are propagated to every Vector Record
                without re-embedding.
              </p>
              {canManage ? (
                <fieldset disabled={saveMetadata.isPending} className="mt-5 min-w-0 space-y-4">
                  <div className="space-y-3">
                    {metadataRows.map((row, index) => (
                      <MetadataEditorRow
                        ...
                        index={index}
                        row={row}
                        onChange={(patch) => updateMetadataRow(row.id, patch)}
                        onRemove={() =>
                          setMetadataRows((rows) =>
                  ...
                  <Button
                    ...
                    disabled={metadataRows.length >= 32}
                  >
                    <Plus />
                    Add field
                  </Button>
                  {parsedMetadata.error ? (
                    <ErrorMessage>{parsedMetadata.error}</ErrorMessage>
                  ) : null}
                  {saveMetadata.error ? (
                    <ErrorMessage>{saveMetadata.error.message}</ErrorMessage>
                  ) : null}
                </fieldset>
              ) : Object.keys(detail.data.customMetadata).length ? (
                <MetadataList
                  className="mt-5"
                  items={Object.entries(detail.data.customMetadata).map(
                    ([key, metadata]) => [key, metadataDisplayValue(metadata)],
                  )}
                />
              ) : (
        ...
        {initialView === "metadata" && canManage ? (
          <SheetFooter className="flex-col-reverse border-t px-5 py-4 sm:flex-row sm:justify-end">
            <Button
              variant="outline"
              className="h-11"
            ...
            >
              ...
              {saveMetadata.isPending ? "Saving…" : "Save metadata"}
            </Button>
          </SheetFooter>
        ) : null}
      </SheetContent>
...
function detailViewTitle(
  ...
  if (view === "chunks")
    return `Chunks${document ? ` · ${document.filename}` : ""}`;
...
function MetadataList({
  ...
  return (
    <dl className={`divide-y border-y text-sm ${className ?? ""}`}>
...
function MetadataEditorRow({
  ...
  index: number;
  row: MetadataDraftRow;
  onChange: (patch: Partial<MetadataDraftRow>) => void;
  onRemove: () => void;
}) {
  const valueId = `vector-metadata-value-${row.id}`;
  return (
    <div className="grid gap-3 rounded-sm border p-3 sm:grid-cols-[minmax(0,1fr)_8rem_minmax(0,1fr)_2.75rem] sm:items-end">
      <div className="space-y-2">
        <Label htmlFor={`vector-metadata-key-${row.id}`}>Key</Label>
        <Input
          id={`vector-metadata-key-${row.id}`}
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(type) =>
            onChange({
              type: type as VectorMetadataType,
      ...
      <div className="space-y-2">
        ...
        {row.type === "boolean" ? (
          <div className="flex h-11 items-center justify-between rounded-md border px-3">
            <span className="text-sm">{row.value ? "True" : "False"}</span>
            <Switch
              id={valueId}
              checked={row.value === true}
              onCheckedChange={(checked) => onChange({ value: checked })}
              aria-label={`Metadata field ${index + 1} boolean value`}
            />
          </div>
        ) : (
          <Input
...
function metadataRowsFrom(metadata: VectorCustomMetadata): MetadataDraftRow[] {
  return Object.entries(metadata).map(([key, item], index) => ({
    id: `${key}-${index}`,
    key,
    type: item.type,
    value: item.type === "boolean" ? item.value : String(item.value),
  }));
}

function metadataFromRows(rows: MetadataDraftRow[]): {
  data?: VectorCustomMetadata;
  error?: string;
} {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (!/^[a-z][a-z0-9_]{0,63}$/.test(key))
      return {
        error: `Field ${index + 1}: use lowercase letters, numbers, and underscores; start with a letter.`,
      };
    if (metadata[key]) return { error: `Metadata key “${key}” is duplicated.` };
    if (row.type === "number") {
      const value = Number(row.value);
      if (row.value === "" || !Number.isFinite(value))
        return { error: `Metadata field “${key}” needs a valid number.` };
      metadata[key] = { type: "number", value };
    } else if (row.type === "boolean")
      metadata[key] = { type: "boolean", value: row.value === true };
    else if (row.type === "date") {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(String(row.value)))
        return { error: `Metadata field “${key}” needs a valid date.` };
...
function Impact({
  ...
  return (
    <div className="bg-background p-3">
      ...
      <dd
        className={
          warning ? "mt-1 font-semibold text-destructive" : "mt-1 font-semibold"
        }
      >
        {value}
      </dd>

...
test("only the dedicated workflow handles RC tags and builds Control", () => {
  ...
  assert.equal(rc.jobs.build.steps.find(s => s.uses?.startsWith("docker/build-push-action")).with.target, "control");
  assert.equal(rc.jobs.build.strategy.matrix.include.length, 2);
